...
export async function POST(
  ...
  try {
    ...
    if (!user || user.role !== 'coordinador') {
      return NextResponse.json({ error: 'Acceso denegado. Se requiere rol de Coordinador.' }, { status: 403 });
    }
    ...
    await auditRepository.log({
      ...
      event_type: `COORDINATOR_EXCUSE_${parseResult.data.action.toUpperCase()}`,
    ...
    console.error('Error in coordinator excuse review controller:', error);
    return NextResponse.json(
